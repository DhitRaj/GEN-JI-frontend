      ...
      <main>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={reveal} transition={{ duration: 0.5 }} className="px-4 py-16 md:px-6">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-2">
              {featured.map((post, i) => (
                <motion.article whileHover={{ y: -5 }} key={post.slug} className={`rounded-2xl border p-8 ${i % 2 === 0 ? 'border-brand card-soft' : 'border-brand card-soft'}`}>
                  ...
                  <div className="mt-5 flex items-center justify-between"><span className="text-sm text-slate-500">{post.date}</span><Link href={`/blog/${post.slug}`} className="font-semibold text-orange-600 hover:text-orange-500">Read More</Link></div>
                </motion.article>
              ))}
            </div>
          </div>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={reveal} transition={{ duration: 0.5 }} className="section-soft px-4 py-16 md:px-6">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {rest.map((post, i) => (
                <motion.article whileHover={{ y: -4 }} key={post.slug} className={`rounded-2xl border p-6 ${i % 2 === 0 ? 'border-brand card-soft' : 'border-brand card-soft'}`}>
                  ...
                  <div className="mt-4 flex items-center justify-between"><span className="text-xs text-slate-500">{post.date}</span><Link href={`/blog/${post.slug}`} className="text-sm font-semibold text-orange-600 hover:text-orange-500">Read</Link></div>
                </motion.article>
              ))}
            </div>
          </div>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={reveal} transition={{ duration: 0.5 }} className="px-4 pb-20 pt-10 text-center md:px-6">
          ...
          <div className="mt-8 flex flex-col justify-center gap-4 sm:flex-row">
            <a href="/contact" className="inline-flex rounded-xl btn-brand px-8 py-4 font-semibold text-white transition hover:bg-orange-600">Get Free Consultation</a>
            <a href="/projects" className="inline-flex rounded-xl border border-brand bg-white/75 px-8 py-4 font-semibold text-brand transition hover:bg-brand-surface-2">View Portfolio</a>
          </div>
        </motion.section>
      </main>
      <Footer />
